			 ...
			 unsigned long bio_flags)
{
	...
	map_length = length;
	ret = btrfs_map_block(fs_info, rw, logical,
			      &map_length, NULL, 0);
	if (ret < 0)
		return ret;
				       ...
				       void *ctx)
{
	...
	if (IS_ERR(root)) {
		if (PTR_ERR(root) == -ENOENT)
			return 0;
		btrfs_debug_with_trace(fs_info,
				      ...
				      int failed_mirror)
{
	...
	if (num_copies == 1) {
		 ...
		 */
		btrfs_debug(fs_info,
			    "%s: cannot repair, num_copies=%d, next_mirror %d, failed_mirror %d",
			    __func__, num_copies, failrec->this_mirror,
			    failed_mirror);
		return 0;
	}
	...
	if (failrec->this_mirror > num_copies) {
		btrfs_debug(fs_info,
			    "%s: (fail) num_copies=%d, next_mirror %d, failed_mirror %d",
			    __func__, num_copies, failrec->this_mirror,
			    failed_mirror);
		return 0;
	}
